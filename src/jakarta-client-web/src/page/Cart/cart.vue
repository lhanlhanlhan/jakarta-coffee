<template>
  <div class="shopping-cart">
    ...
    <div class="store-content page-cart">
      <div v-if="cartList.length" class="gray-box">
          ...
          <div class="ui-cart">
            <div>
              ...
              <div class="cart-table" v-for="(item, i) in cartList" :key="i">
                <div class="cart-group divide pr" :data-productid="item.item_id">
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      ...
                      <div class="items-thumb fl">
                        <img :alt="item.name"
                             :src="item.photo_uri === 'null' ? basicFoodUrl : item.photo_uri">
                        <a href="javascript:;" :title="item.name" target="_blank"></a>
                      </div>
                      <!--信息-->
                      <div class="name hide-row fl">
                        <div class="name-table">
                          <a href="javascript:;" :title="item.name" target="_blank"
                             v-text="item.name"></a>
                          <ul class="attribute">
                            <li>白色</li>
                          </ul>
                      ...
                      <!--删除按钮-->
                      <div class="operation">
                        <a class="items-delete-btn" @click="delFromCart(item.item_id)"></a>
                      </div>
                      <!--商品数量-->
                      <div>
                        <!--总价格-->
                        <div class="subtotal" style="font-size: 14px">$ {{(item.price * item.item_count).toFixed(2)}}</div>
                        <!--数量-->
                        <buy-num :num="item.item_count"
                                 :id="item.item_id"
...
<script>
  ...
  import { changeCart, delFromCart } from '../../api/cart'
  export default {
    data () {
      return {
    ...
    computed: {
      ...
      // 计算总数量
      totalNum () {
        return this.cartList.reduce((accumulator, item) => accumulator + item.item_count, 0)
      },
      // 选中的总价格
      checkPrice () {
        return this.cartList.reduce((a, item) => item.checked ? a + (item.item_count * item.price) : a, 0)
      },
      // 选中的商品数量
      checkNum () {
        return this.cartList.reduce((accumulator, item) => item.checked ? accumulator + item.item_count : accumulator, 0)
      }
    },
    methods: {
      ...
      // 设置某个项目是否选中
      editCheck (productId, productNum, checked) {
        this.EDIT_CART(
          {
            productId,
            productNum,
      ...
      editCart (type, item) {
        if (type && item) {
          // 商品信息
          let { checked, item_id, item_count } = item
          if (type === 'check') {
            // 勾选
            this.editCheck(item_id, item_count, !checked)
          } else if (type === 'count') {
            // 修改数量
            this.editNum(item_count, item_id, checked)
          }
        } else {
          console.log('缺少所需参数')
        }
      ...
      delFromCart (productId) {
        delFromCart({
          item_id: productId
        }).then(result => this.EDIT_CART({productId}))
      },
      // TODO - 结算
      checkout () {
